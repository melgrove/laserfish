...
function squareTranslation(initial: string, XYChange: Array<number>): string | null {
...
function getSquaresBetween(orig: string, dest: string) {
    // console.log(orig)
    // console.log(dest)
    ...
    while(newSquare !== dest) {
        squares.push(newSquare as string);
        newSquare = squareTranslation(newSquare as string, direction);
    }
    return squares;
}

export function getLegalMoves(position: Position, turn: Color) {
    const dests: Dests = [];

    for(let square in position) {
        const piece = position[square];
        // Only find for current player turn
        if(piece[1] !== turn) continue;
        ...
        switch (piece[0]) {
            ...
            case "n":
                knightChangeCoords
                    ...
                    .forEach(e => dests.push([square, e as string, false]));
                break;
            ...
            case "q":
                ...
                for(let direction of diagonalChangeCoords) {
                    let prevPos = square;
                    let currentPos = squareTranslation(square, direction);
                    let isValidDirection: boolean = false;
...
export function getResult(position: Position): 'w' | 'b' | 'd' | null {
...
export function newPositionFromMove(position: Position, move: Dest): Position {
...
export function fenToPosition(fen: string): { position: Position, turn: Color } {
    const position: Position = {};
    const fenArray: Array<string> = fen.split(' ');
    ...
    return {
        position,
        turn: fenArray[1] as Color
    };
}

export function positionToFen(position: Position, turn: Color) {
    const fenArray = [];
    for(let m = 8; m >= 1; m--) {
